...
- `pixel_identical` — совпадают ли декодированные RGBA-пиксели;
- `changed_pixels` — количество изменённых пикселей;
- `max_channel_delta` — максимальное отличие цветового канала;
- `bytes_identical` — совпадают ли исходные PNG побайтово;
- SHA-256 обоих PNG.

Побайтовое отличие PNG допустимо только тогда, когда `pixel_identical=true`. Это учитывает различия способа упаковки PNG без ложной визуальной регрессии.
