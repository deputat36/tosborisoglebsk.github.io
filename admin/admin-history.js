(function(){
  ...
  function restoreLatest(){
    if(typeof state === 'undefined' || typeof save !== 'function' || typeof current !== 'function') return;
    const section = state.section;
    const history = readHistory(section);
    ...
    if(!confirm(`Восстановить снимок раздела «${section}» от ${dateLabel}? Текущее состояние будет сохранено перед откатом.`)) return;
