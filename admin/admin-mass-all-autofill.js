(function(){
  ...
  function readyMassAll(){
    const row=document.querySelector('.tools-row');
    if(!row)return;
    row.insertAdjacentHTML('beforeend','<button class="btn" id="massFillAll">Автозаполнить все поля и логотипы для ТОСов</button><button class="btn" id="massCsvNoLogo">CSV без логотипа</button>');
    document.querySelector('#massFillAll')?.addEventListener('click',()=>{
    ...
    document.querySelector('#massCsvNoLogo')?.addEventListener('click',()=>{
      if(typeof state==='undefined'||state.section!=='toses')return;
      const rows=current().filter(item=>!item.logo).map(item=>[item.name||'',item.slug||'',item.location||'',item.chairperson||'',`/assets/img/tos-logos/${item.slug||''}.svg`]);
      const header=['ТОС','slug','Территория','Председатель','Рекомендуемый путь логотипа'];
      const csv=[header,...rows].map(r=>r.map(esc).map(v=>'"'+v+'"').join(';')).join('\n');
      downloadText('tos-no-logo.csv','\ufeff'+csv,'text/csv;charset=utf-8');
      msg([`CSV сформирован. Записей без логотипа: ${rows.length}`],true);
    });
  }
  ready(()=>{
    const wait=setInterval(()=>{if(typeof state==='undefined')return;clearInterval(wait);readyMassAll();},50);
  });
})();