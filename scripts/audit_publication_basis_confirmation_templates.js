...
const ALLOWED_RESPONSE_FIELDS = new Set([
  ...
  'response_status',
  'chairperson_status',
  'published_field_types_to_keep',
  'published_field_types_to_remove',
...
function main() {
  ...
  for (const field of ALLOWED_RESPONSE_FIELDS) {
    if (!responseFields.has(field)) errors.push(`response_fields is missing ${field}`);
  }
  for (const field of responseFields) {
    if (!ALLOWED_RESPONSE_FIELDS.has(field)) errors.push(`response_fields must not include unapproved field ${field}`);
  }
  ...
  if (/https?:\/\//i.test(serialized)) errors.push('confirmation templates must not contain raw URLs');
