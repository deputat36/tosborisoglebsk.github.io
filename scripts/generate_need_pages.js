...
function makePage(item, toses) {
  ...
  const origin = inferContentOrigin(item, 'needs');
...
<body>
  ...
  <main id="main">
    <section class="hero"><div class="container hero-card"><a class="chip" href="/needs/">← Все потребности</a><div class="eyebrow">${esc(item.need_type || 'Потребность')} · ${esc(dateRu(item.date))}</div><div class="meta"><span class="tag ${esc(originClass)}">${esc(originLabel)}</span></div><h1>${esc(title)}</h1><p class="lead">${esc(description)}</p><div class="hero-actions"><a class="btn primary" href="/contacts/">Предложить помощь</a>${tos ? `<a class="btn" href="/tos/${esc(tos.slug)}/">ТОС «${esc(tos.name)}»</a>` : ''}<a class="btn" href="/partners/">Партнёрам</a><a class="btn" href="/done/">Истории результата</a></div></div></section>
