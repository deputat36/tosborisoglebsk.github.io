...
function testAnchors() {
  ...
  try {
    ...
    const broken = fs.readFileSync(path.join(dir, 'index.html'), 'utf8').replace('#section', '#missing');
    write(path.join(dir, 'index.html'), broken);
...
function testIntegration() {
  ...
  for (const token of ['якорей', 'title', 'description', 'нулевыми', 'site_health.json']) {
