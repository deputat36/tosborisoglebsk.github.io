...
<main id="main">
  ...
  <section class="section" data-pages-manual-check><div class="container section-head"><div><h2>Ручная проверка Settings → Pages</h2><p>Готовый протокол для закрытия issue #164 без подмены ручного просмотра автоматическим smoke-тестом.</p></div><a class="btn primary" href="/data/github_pages_manual_check_template.csv">Скачать шаблон CSV</a></div><div class="container grid">
    ...
    <article class="card"><div class="card-inner"><h3>3. Фиксация результата</h3><p>После фактической проверки добавить отдельную строку <code>actions-013</code> в <code>data/actions_diagnostics.csv</code>. Статус <code>passed</code> допустим только после проверки всех обязательных пунктов.</p><p><a class="btn" href="/docs/GITHUB-PAGES-MANUAL-CHECK.md">Полная инструкция</a></p></div></article>
  </div><div class="container notice"><b>Важно:</b> незаполненный шаблон со статусами <code>not_checked</code> не является evidence, не переводит <code>actions-011</code> в <code>passed</code> и не закрывает issue #164.</div></section>
