...
function main() {
  ...
  if (!Number.isInteger(pages.seo_warnings_count)) {
    errors.push('pages.seo_warnings_count must be an integer');
  }
  if (!Number.isInteger(pages.broken_internal_links_count)) {
    errors.push('pages.broken_internal_links_count must be an integer');
  }
  ...
  if (pages.seo_warnings_count !== 0) {
    errors.push(`pages.seo_warnings_count must be zero, got ${pages.seo_warnings_count}`);
  }
  if (pages.broken_internal_links_count !== 0) {
    errors.push(`pages.broken_internal_links_count must be zero, got ${pages.broken_internal_links_count}`);
  }
  if (seoWarnings && seoWarnings.length !== 0) {
    errors.push(`seo_warnings must be empty, got ${seoWarnings.length}`);
  }
  if (brokenLinks && brokenLinks.length !== 0) {
    errors.push(`broken_internal_links must be empty, got ${brokenLinks.length}`);
  }

  if (errors.length) {
    throw new Error(`Site health quality gate failed:\n${errors.join('\n')}`);
  }
