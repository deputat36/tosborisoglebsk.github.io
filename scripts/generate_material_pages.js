...
function relatedLinks(article){
  return arr(article.related_links).filter((link) => link && typeof link === 'object' && String(link.label || '').trim() && String(link.href || '').trim());
}
function makePage(article){
  ...
  const tools = relatedLinks(article);
  ...
  const contextLinks = buildCollectionContextLinks('articles', article, null, origin);
  const contextSectionId = collectionContextSectionId('articles');
  const schema = {
  ...
  const toolsMarkup = tools.length
    ? `<hr class="sep"/><h2>Связанные инструменты</h2><div class="card-actions">${tools.map((link) => `<a class="btn" href="${esc(link.href)}">${esc(link.label)}</a>`).join('')}</div>`
    : '';
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${esc(title)} | Материалы ТОС БГО</title><meta name="description" content="${esc(description)}"/><meta name="theme-color" content="#2f7d5a"/><link rel="canonical" href="${esc(canonical)}"/><meta property="og:title" content="${esc(title)}"/><meta property="og:description" content="${esc(description)}"/><meta property="og:type" content="article"/><meta property="og:url" content="${esc(canonical)}"/><meta property="og:image" content="${SITE_URL}/assets/img/og-cover.svg"/><link rel="icon" href="/favicon.svg" type="image/svg+xml"/><link rel="manifest" href="/site.webmanifest"/><link rel="stylesheet" href="/assets/css/styles.css"/><script type="application/ld+json">${JSON.stringify(schema)}</script></head><body><a class="skip-link" href="#main">Перейти к содержимому</a><header class="header"><div class="container header-inner"><a class="brand" href="/"><img src="/assets/img/logo.svg" alt="ТОС БГО"/></a><nav class="nav" id="site-nav" aria-label="Навигация"><a href="/tos/">Каталог ТОС</a><a href="/news/">Новости</a><a href="/grants/">Конкурсы</a><a href="/projects/">Проекты</a><a href="/calendar/">Календарь</a><a href="/needs/">Нужна помощь</a><a href="/materials/">Материалы</a><a href="/documents/">Документы</a><a href="/create-tos/">Как создать ТОС</a><a href="/contacts/">Контакты</a></nav><div class="actions"><a class="btn" href="/search/">Поиск</a><button class="btn menu-btn" type="button" data-action="menu" aria-expanded="false" aria-controls="site-nav">Меню</button><button class="btn" type="button" data-action="theme">Тема</button></div></div></header><main id="main"><section class="hero"><div class="container hero-card"><a class="chip" href="/materials/">← Материалы</a><div class="eyebrow">${esc(category)}</div><div class="meta"><span class="tag ${esc(originClass)}">${esc(originLabel)}</span></div><h1>${esc(title)}</h1><p class="lead">${esc(visibleLead)}</p></div></section><section class="section tight"><div class="container notice"><b>Статус материала:</b> ${esc(originNotice)}</div></section><section class="section"><div class="container prose">${content.length ? content.map(p => `<p>${esc(p)}</p>`).join('') : '<p>Материал готовится к публикации.</p>'}${toolsMarkup}<hr class="sep"/><div class="card-actions"><a class="btn" href="/materials/">Все материалы</a><a class="btn" href="/documents/">Документы</a><a class="btn" href="/create-tos/">Как создать ТОС</a></div></div></section><section class="section tight" id="${esc(contextSectionId)}" aria-labelledby="${esc(contextSectionId)}-title"><div class="container prose"><h2 id="${esc(contextSectionId)}-title">Что делать дальше</h2><ul>${contextLinks.map((link) => `<li><a href="${esc(link.href)}">${esc(link.label)}</a></li>`).join('')}</ul><p class="tiny">Ссылки подобраны по теме инструкции и ведут к действующим рабочим разделам портала.</p></div></section></main><footer class="footer"><div class="container footer-grid"><div><b>Портал ТОС БГО</b><div class="tiny">© <span id="year"></span> tosborisoglebsk.ru</div></div><div class="tiny">Страница материала создана автоматически из data/articles.json.</div></div></footer><script src="/assets/js/site.js"></script></body></html>`;
}
...
module.exports = { relatedLinks, makePage };
