...
const GENERATED_PAGE_MARKER = 'Страница материала создана автоматически из data/articles.json.';
...
function main(){
  ...
  const removed = removeStaleGeneratedDirectories({
    ...
    validIds: articles.map((article) => article.id),
    marker: GENERATED_PAGE_MARKER
  });
  articles.forEach(article => write(path.join(ROOT, 'materials', article.id, 'index.html'), makePage(article)));
  updateSitemap(articles);
  console.log(`Generated material pages: ${articles.length}; removed stale: ${removed.length}`);
}
main();
