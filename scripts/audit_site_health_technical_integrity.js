...
function main() {
  const errors = [];
  contractPathKeys.forEach((key) => {
    if (!fs.existsSync(paths[key])) errors.push(`missing ${key}: ${path.relative(ROOT, paths[key])}`);
  });

  const dataAvailable = dataPathKeys.every((key) => fs.existsSync(paths[key]));
  if (strict && !dataAvailable) {
    dataPathKeys.forEach((key) => {
      if (!fs.existsSync(paths[key])) errors.push(`strict mode missing ${key}: ${path.relative(ROOT, paths[key])}`);
    });
  }
  ...
  if (dataAvailable) {
    ...
    integrity = siteHealth.technical_integrity;
  ...
  if (integrity) {
    ...
    console.log('Site health technical integrity contract OK: generated report is not required in this workflow');
  }
}
