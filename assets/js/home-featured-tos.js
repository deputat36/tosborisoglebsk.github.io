...
const featuredTosUrl = (value, fallback) => {
  const url = String(value || fallback || '#').trim();
  return featuredTosEsc(url);
};
...
async function renderFeaturedTos() {
  ...
  try {
    ...
    const primaryUrl = featuredTosUrl(item.cta_primary_url, '/tos/');
    const secondaryUrl = featuredTosUrl(item.cta_secondary_url, '/done/');
    const thirdUrl = featuredTosUrl(item.cta_third_url, '/contacts/');

    root.innerHTML = `<article class="card full"><div class="card-inner"><div class="meta"><span class="tag warn">${featuredTosEsc(item.label || 'Активная территория')}</span>${item.updated_at ? `<span class="tag">Обновлено: ${featuredTosEsc(item.updated_at)}</span>` : ''}</div><h3>${featuredTosEsc(item.title || 'ТОС')}</h3><p class="lead">${featuredTosEsc(item.subtitle || '')}</p><p>${featuredTosEsc(item.description || '')}</p>${reasons.length ? `<div class="notice"><b style="color:var(--text)">Почему выделяем</b><ul>${reasons.map((reason) => `<li>${featuredTosEsc(reason)}</li>`).join('')}</ul></div>` : ''}<p class="tiny"><b>Источник:</b> ${item.source_url ? `<a href="${featuredTosUrl(item.source_url, '#')}">${featuredTosEsc(item.source_label || 'Источник')}</a>` : featuredTosEsc(item.source_label || 'Редакция портала')}</p><div class="card-actions"><a class="btn primary" href="${primaryUrl}">${featuredTosEsc(item.cta_primary_text || 'Открыть карточку')}</a><a class="btn" href="${secondaryUrl}">${featuredTosEsc(item.cta_secondary_text || 'Посмотреть истории')}</a><a class="btn" href="${thirdUrl}">${featuredTosEsc(item.cta_third_text || 'Прислать материал')}</a></div></div></article>`;
  } catch (error) {
