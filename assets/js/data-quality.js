...
function renderQualitySummary(summary) {
  ...
  root.innerHTML = [
    ...
    qualityStat(summary.without_needs || 0, 'без потребностей'),
    qualityStat(summary.without_projects || 0, 'без проектов')
  ].join('');
}
...
function renderQualityList(items) {
  ...
  const important = items
    ...
    .sort((a, b) => Number(a.score || 0) - Number(b.score || 0) || String(a.name).localeCompare(String(b.name), 'ru'));
  ...
  root.innerHTML = important.map((item) => {
    ...
    return `<article class="list-item"><div class="meta"><span class="tag ${priorityClass(item.priority)}">${qualityEsc(item.priority)} приоритет</span><span class="tag">${qualityEsc(item.score)}%</span><span class="tag">${qualityEsc(item.location || 'территория уточняется')}</span></div><h3>ТОС «${qualityEsc(item.name || item.slug)}»</h3><p><b>Председатель:</b> ${qualityEsc(item.chairperson || 'уточняется')}</p><div class="audit-missing">${missing || '<span class="tag ok">Основные поля заполнены</span>'}</div>${recommendations ? `<ul class="tiny">${recommendations}</ul>` : ''}<div class="card-actions"><a class="btn" href="/tos/${qualityEsc(item.slug)}/">Открыть карточку</a><a class="btn" href="/update-tos/?tos=${encodeURIComponent(item.slug || '')}">Прислать уточнение</a></div></article>`;
  }).join('');
}
