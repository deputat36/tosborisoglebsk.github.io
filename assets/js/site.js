...
async function renderDocs(){ const root=$('#documents-list'); if(!root) return; const data=(await getJSON('/data/documents.json')).filter(isPublished); root.innerHTML=data.map(d=>`<article class="list-item ${d.type==='Шаблон'?'template-card':''}"><div class="meta"><span class="tag">${esc(d.type)}</span><span class="tag">${esc(d.date)}</span></div><h3>${esc(d.title)}</h3><p>${esc(d.description)}</p>${d.url?`<a class="btn" href="/${esc(d.url)}" target="_blank" rel="noopener">Открыть документ</a>`:''}</article>`).join(''); }
async function renderGrants(){ const root=$('#grants-list'); if(!root) return; const data=await getJSON('/data/grants.json'); root.innerHTML=data.map(g=>`<article class="list-item"><div class="meta"><span class="tag warn">${esc(g.status)}</span><span class="tag">${esc(g.amount)}</span></div><h3>${esc(g.title)}</h3><p><b>Срок:</b> ${esc(g.deadline)}</p><p><b>Кому:</b> ${esc(g.who)}</p><p><b>Направления:</b> ${esc(g.directions)}</p><p>${esc(g.note)}</p>${g.source?`<a class="btn" target="_blank" rel="noopener" href="${esc(g.source)}">Источник</a>`:''}</article>`).join(''); }
async function renderProjects(){ const root=$('#projects-list'); if(!root) return; const data=(await getJSON('/data/projects.json')).filter(isPublished); root.innerHTML=data.map(p=>`<article class="card"><div class="card-inner"><div class="tag">${esc(p.type)}</div><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p>${p.tos_slug?`<p class="tiny"><b>Привязка к ТОС:</b> ${esc(p.tos_slug)}</p>`:''}<hr class="sep"/><ul class="tiny">${(p.steps||[]).map(s=>`<li>${esc(s)}</li>`).join('')}</ul></div></article>`).join(''); }

async function renderSearch(){
  const root=$('#search-results'), input=$('#site-search'); if(!root||!input) return;
  const [t,n,a,d,g,p,e,needs] = await Promise.all([
    getJSON('/data/toses.json'), getJSON('/data/news.json'), getJSON('/data/articles.json'), getJSON('/data/documents.json'), getJSON('/data/grants.json'), getJSON('/data/projects.json'),
    getJSON('/data/events.json').catch(()=>[]), getJSON('/data/needs.json').catch(()=>[])
  ]);
  const all=[
    ...t.filter(isPublished).map(x=>({type:'ТОС',title:'ТОС «'+x.name+'»',text:[x.location,x.boundaries,x.chairperson,x.description].join(' '),url:'/tos/'+x.slug+'/'})),
    ...n.filter(isPublished).map(x=>({type:'Новость',title:x.title,text:[x.lead,(x.text||[]).join(' ')].join(' '),url:'/news/'+x.id+'/'})),
    ...a.filter(isPublished).map(x=>({type:'Материал',title:x.title,text:[x.lead,(x.content||[]).join(' ')].join(' '),url:'/materials/'+x.id+'/'})),
    ...d.filter(isPublished).map(x=>({type:'Документ',title:x.title,text:x.description,url:x.url?'/'+x.url:'/documents/'})),
    ...g.map(x=>({type:'Конкурс',title:x.title,text:[x.note,x.directions,x.who].join(' '),url:'/grants/'})),
    ...p.filter(isPublished).map(x=>({type:'Проект',title:x.title,text:[x.type,x.description,(x.steps||[]).join(' ')].join(' '),url:x.tos_slug?'/tos/'+x.tos_slug+'/':'/projects/'})),
    ...e.filter(isPublished).map(x=>({type:'Событие',title:x.title,text:[x.type,x.description,x.place,x.tos_slug].join(' '),url:'/calendar/'})),
...
function projectSmall(p){ return `<article class="card"><div class="card-inner"><div class="tag">${esc(p.type||'Проект')}</div><h3>${esc(p.title)}</h3><p>${esc(p.description||'')}</p>${(p.steps||[]).length?`<hr class="sep"/><ul class="tiny">${p.steps.slice(0,5).map(s=>`<li>${esc(s)}</li>`).join('')}</ul>`:''}</div></article>`; }
...
async function renderTosDetail(){
  ...
  try{
    const [toses, news, projects, needs] = await Promise.all([
      getJSON('/data/toses.json'), getJSON('/data/news.json').catch(()=>[]), getJSON('/data/projects.json').catch(()=>[]), getJSON('/data/needs.json').catch(()=>[])
    ]);
