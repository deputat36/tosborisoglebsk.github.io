...
async function renderSearch(){
  ...
  const [t,n,a,d,g,p] = await Promise.all([getJSON('/data/toses.json'),getJSON('/data/news.json'),getJSON('/data/articles.json'),getJSON('/data/documents.json'),getJSON('/data/grants.json'),getJSON('/data/projects.json')]);
  const all=[...t.filter(isPublished).map(x=>({type:'ТОС',title:'ТОС «'+x.name+'»',text:[x.location,x.boundaries,x.chairperson,x.description].join(' '),url:'/tos/'+x.slug+'/'})),...n.filter(isPublished).map(x=>({type:'Новость',title:x.title,text:[x.lead,(x.text||[]).join(' ')].join(' '),url:'/news/'+x.id+'/'})),...a.filter(isPublished).map(x=>({type:'Материал',title:x.title,text:[x.lead,(x.content||[]).join(' ')].join(' '),url:'/materials/'+x.id+'/'})),...d.filter(isPublished).map(x=>({type:'Документ',title:x.title,text:x.description,url:x.url?'/'+x.url:'/documents/'})),...g.map(x=>({type:'Конкурс',title:x.title,text:[x.note,x.directions,x.who].join(' '),url:'/grants/'})),...p.filter(isPublished).map(x=>({type:'Проект',title:x.title,text:x.description,url:'/projects/'}))];
...
async function renderTosDetail(){
  ...
  try{
    const [toses, news, projects] = await Promise.all([getJSON('/data/toses.json'), getJSON('/data/news.json').catch(()=>[]), getJSON('/data/projects.json').catch(()=>[])]);
