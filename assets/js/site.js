(function(){
  ...
  function initTosList(){
    ...
    function render(){
      ...
      list.innerHTML = filtered.map(item=>{
        const href = item.id ? (item.id + "/") : ("view.html?id=" + encodeURIComponent(item.id));
        const badges = []
  ...
  function initTosView(){
    ...
    // SEO: перенаправление со старого URL (view.html?id=...) на «красивый» /tos/{id}/
    if(location.pathname.endsWith("/tos/view.html") && item.id){
