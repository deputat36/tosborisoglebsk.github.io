...
const updateLink = (type = 'card') => `/update-tos/?type=${encodeURIComponent(type)}#message-builder`;
...
async function renderSearch() {
  ...
  try {
    ...
    const items = [
      ...
      { type: 'Маршруты', title: 'Практические маршруты ТОС БГО', text: 'единая карта действий маршруты жителю председателю партнеру инициативной группе проект потребность событие результат создать ТОС с чего начать', url: '/action-routes/' },
      ...
      { type: 'Все разделы', title: 'Все разделы сайта ТОС БГО', text: 'разделы навигатор каталог жители председателю партнеры проекты документы новости контакты поиск карта материалы правовая основа практические маршруты', url: '/sections/' },
