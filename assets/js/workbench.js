document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCardPreview(slug) {
    ...
    body.innerHTML = `<div class="meta"><span class="tag ${item.priority === 'Высокий' ? 'warn' : ''}">${esc(item.priority || 'Приоритет уточняется')}</span><span class="tag">${esc(item.score || 0)}%</span><span class="tag">slug: ${esc(slug)}</span></div><h2>Предпросмотр: ТОС «${esc(tos.name || item.name || slug)}»</h2><div class="kpi"><div class="tile"><b>${valueOrEmpty(tos.type)}</b><span>тип ТОС</span></div><div class="tile"><b>${valueOrEmpty(tos.population)}</b><span>жителей</span></div><div class="tile"><b>${valueOrEmpty(tos.founded)}</b><span>год создания</span></div></div><hr class="sep"/><div class="grid"><article class="card"><div class="card-inner"><h3>Территория</h3><p><b>Место:</b> ${valueOrEmpty(tos.location || item.location)}</p><p><b>Границы:</b> ${valueOrEmpty(tos.boundaries)}</p></div></article><article class="card"><div class="card-inner"><h3>Контакты</h3><p><b>Председатель:</b> ${valueOrEmpty(tos.chairperson)}</p><p><b>Телефон:</b> ${listOrEmpty(tos.phones)}</p><p><b>Email:</b> ${listOrEmpty(tos.emails)}</p></div></article><article class="card"><div class="card-inner"><h3>Публичность</h3><p><b>Соцсети:</b> ${listOrEmpty(tos.social_links)}</p><p><b>Логотип:</b> ${tos.logo ? esc(tos.logo) : 'нужно добавить'}</p><p><b>Обновлено:</b> ${valueOrEmpty(tos.updated_at)}</p></div></article></div><hr class="sep"/><p>${esc(description)}</p><div class="meta">${missing || '<span class="tag ok">Критичных пробелов в аудите не указано</span>'}</div><div class="card-actions"><a class="btn" href="/tos/${encodeURIComponent(slug)}/">Открыть карточку</a><a class="btn" href="/data-requests/">Открыть сообщение</a><a class="btn primary" href="${updateUrl(slug)}">Уточнить данные</a></div>`;
