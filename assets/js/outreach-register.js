document.addEventListener('DOMContentLoaded', () => {
  ...
  function validationIssues(item) {
    ...
    const sentStatuses = ['sent', 'waiting', 'follow_up', 'received', 'closed'];
    const resultStatuses = ['received', 'closed', 'resolved'];
    ...
    if (sentStatuses.includes(item.status) && !item.channel) issues.push('для статуса нужен канал');
    if (sentStatuses.includes(item.status) && !sent) issues.push('для статуса нужна дата отправки');
    if (['waiting', 'follow_up'].includes(item.status) && !followUp) issues.push('не указана дата повторного контакта');
    if (resultStatuses.includes(item.status) && !response) issues.push('не указана дата результата или ответа');
    if (resultStatuses.includes(item.status) && !item.response_source) issues.push('не указан источник результата или ответа');
  ...
  function isOverdue(item) {
    if (!item.follow_up_date || ['received', 'closed', 'resolved'].includes(item.status)) return false;
  ...
  function renderStats() {
    ...
    const values = [
      ...
      ['Повторный контакт', rows.filter(isOverdue).length],
  ...
  function renderTable() {
    ...
    root.innerHTML = `<div class="table-wrap"><table><thead><tr><th>Задача</th><th>Группа</th><th>Статус</th><th>Канал</th><th>Даты</th><th>Контроль</th></tr></thead><tbody>${items.map((item) => {
      ...
      const validation = issues.length ? `<div class="meta"><span class="tag warn">Ошибка данных</span></div><div class="tiny">${issues.map(esc).join('; ')}</div>` : '<div class="tiny">структура строки корректна</div>';
      return `<tr><td><b>${esc(item.subject)}</b><div class="tiny">${esc(item.outreach_id)} · ${esc(item.source_request_id)}</div></td><td>${esc(groupLabels[item.request_group] || item.request_group)}</td><td><span class="tag ${overdue || item.status === 'follow_up' ? 'warn' : ''}">${esc(overdue ? 'Просрочен повтор' : (statusLabels[item.status] || item.status))}</span></td><td>${esc(item.channel || (item.status === 'resolved' ? 'обращение не потребовалось' : 'не определён'))}<div class="tiny">${esc(item.contact || '')}</div></td><td>${dates}</td><td>${validation}<b>${esc(item.blocker || 'нет блокера')}</b><div class="tiny">${esc(item.next_step || '')}</div></td></tr>`;
